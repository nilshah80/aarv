...
- [ ] **Stage 1** — Cut the aarv root release as **`v0.9.0`** (minor bump; intentional breaking changes documented in `CHANGELOG.md` and `docs/MIGRATION_v0.9.md`). Run the §12.6.8 release checklist end-to-end (root tests, lint, govulncheck, tag, `go list -m` verification from outside the repo). *Implementation complete; tag push pending Checkpoint B approval.*
